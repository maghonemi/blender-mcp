...
# Rigging handlers - Skinning and Auto-rig (optional)
SKINNING_AVAILABLE = False
try:
    from handlers.rigging.skinning import (
        AutoWeightAssignHandler
    )
    SKINNING_AVAILABLE = True
    logger.info("Skinning handlers imported successfully")
except ImportError as e:
    logger.warning(f"Could not import skinning handlers: {e}")
except Exception as e:
    logger.error(f"Error importing skinning handlers: {e}")

AUTO_RIG_AVAILABLE = False
try:
    from handlers.rigging.auto_rig import (
        RigHandHandler,
        RigBodyHandler
    )
    AUTO_RIG_AVAILABLE = True
    logger.info("Auto-rig handlers imported successfully")
except ImportError as e:
    logger.warning(f"Could not import auto-rig handlers: {e}")
except Exception as e:
    logger.error(f"Error importing auto-rig handlers: {e}")
...
def register_all_handlers():
    ...
    # Rigging handlers - Skinning and Auto-rig (optional)
    if SKINNING_AVAILABLE:
        try:
            command_router.register_handler(AutoWeightAssignHandler())
            logger.info("Skinning handlers registered")
        except Exception as e:
            logger.warning(f"Could not register skinning handlers: {e}")

    if AUTO_RIG_AVAILABLE:
        try:
            command_router.register_handler(RigHandHandler())
            command_router.register_handler(RigBodyHandler())
            logger.info("Auto-rig handlers registered")
        except Exception as e:
            logger.warning(f"Could not register auto-rig handlers: {e}")
